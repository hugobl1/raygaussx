...
if __name__ == "__main__":
    ...
    if not((args.ply_path is not None) ^ (args.output is not None)):# XOR
        raise ValueError("You must provide either the output folder or the point cloud path")
    ...
    if gui_mode: #Case where we have an output file
        ...
        path_output,_ = os.path.split(args.ply_path)
    ...
    with open(camera_path_filename, "r", encoding="utf-8") as f:
        ...
        for camera in camera_path["camera_path"]:
            ...
            cam_list.append(PerspectiveCamera(c2w,cam_center,image_width,image_height,fovx,fovy,0.5,0.5,0.01,1000.0))
    #To device
    ...
    if gui_mode:
        image_list=test.render(pointcloud,cam_list,config.training.max_prim_slice,config.scene.dt,rnd_sample=config.training.rnd_sample,supersampling=(config.training.supersampling_x,config.training.supersampling_y), white_background=config.scene.white_background)
    else:
        image_list=test.render(pointcloud,cam_list,max_prim_slice=1024,dt_step=config.scene.dt,rnd_sample=0,supersampling=(1,1), white_background=False)
    ############################################################################################################
    ...
    os.system("ffmpeg -framerate 60 -i "+os.path.join(path_images,"im%06d.png")+" -c:v libx264 -crf 20 "+os.path.join(path_video,args.name_video+".mp4"))