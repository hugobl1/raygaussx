...
class RenderOptixSettings:
    def __init__(self, context,update, gas, program_grps_fwd, pipeline_fwd,
                 program_grps_bwd, pipeline_bwd, dt_step,dynamic_sampling,viewpoint_cam,
...
class RenderOptixFunction(torch.autograd.Function):
    ...
    def forward(ctx,positions,scales,normalized_quaternions,densities,color_features,
      ...
      # bboxes = u_ox.ellipsoids_bbox_from_quat_old(cp_quaternions,cp_positions,cp_scales,cp_densities)
      bboxes, bb_min, bb_max = u_ox.ellipsoids_bbox_from_quat(cp_quaternions,cp_positions,cp_scales,cp_densities)
      ...
      cp_rgb=compute_cupy_rgb(settings.viewpoint_cam.camera_center,cp_positions,cp_color_features,
                ...
                order_sh)
      cp_ray_colors= u_ox.launch_pipeline_forward(settings.pipeline_fwd,sbt_fwd, settings.gas,bb_min,bb_max,settings.dt_step,
    ...
    def backward(ctx, dloss_dray_colors):
      ...
      return (positions_grad,scales_grad,normalized_quaternions_grad,densities_grad,color_features_grad,sph_gauss_feat_grad,bandwidth_sharpness_grad,lobe_axis_grad,None)     
