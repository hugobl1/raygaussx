...
cuda_sh_gui = cupy_module.get_function('gui_sh')
...
def compute_cupy_rgb(camera_center,cupy_positions,cupy_color_features,
                          ...
                          degree_sh,num_prim,max_sh_degree,max_sg_display):
    max_num_sh=(max_sh_degree+1)**2
    num_points=num_prim
    block_size=128
    num_blocks=(num_points+block_size-1)//block_size
    # cupy_camera_center=cp.from_dlpack(to_dlpack(camera_center.contiguous()))
    cupy_camera_center=cp.array(camera_center,dtype=cp.float32)
    cp_colors_rgb=cp.zeros((num_points,3),dtype=cp.float32)
    cuda_sh_gui((num_blocks,),(block_size,),
