...
class Scene:
    ...
    def __init__(self, config , pointcloud : point_cloud.PointCloud, shuffle=False, train_resolution_scales=[1.0], test_resolution_scales=[1.0],init_pc=True):
        log_phase(logger, "Scene Initialization")
        ...
        elif os.path.exists(os.path.join(config.scene.source_path, "transforms.json")):
            scene_info = sceneLoadTypeCallbacks["Blender"](config.scene.source_path, config.scene.white_background, config.scene.eval,config)
        ...
        elif os.path.exists(os.path.join(config.scene.source_path, "bbox.txt")):
            scene_info = sceneLoadTypeCallbacks["NSVF"](config.scene.source_path, config.scene.white_background, config.scene.eval,config)
        else:
