...
class OptimizerManager:
    ...
    def _reorder_tensors(self, reorder_indexes):
        for group in self.optimizer.param_groups:
            stored_state = self.optimizer.state.get(group['params'][0], None)
            if stored_state is not None:
                #stored_state["square_avg"] = stored_state["square_avg"][reorder_indexes]
                stored_state["exp_avg"] = stored_state["exp_avg"][reorder_indexes]
                stored_state["exp_avg_sq"] = stored_state["exp_avg_sq"][reorder_indexes]
                del self.optimizer.state[group['params'][0]]
                group["params"][0] = (group["params"][0][reorder_indexes].requires_grad_(True))
                self.optimizer.state[group['params'][0]] = stored_state
            else:
                group["params"][0] = (group["params"][0][reorder_indexes].requires_grad_(True))
        return group["params"][0]
...
class OptimizerManagerDict:
    ...
    def get_lrs(self) -> dict:
        """
        Retourne un dict {name: lr} pour chaque optimizer géré.
        """
        return {
            name: opt.get_lr()
            for name, opt in self.optimizer_manager_dict.items()
        }
    def print_state(self):
        for optimizer_manager in self.optimizer_manager_dict.values():
    ...
    def _reorder_tensors(self, reorder_indexes):
        dict={}
        for name, optimizer_manager in self.optimizer_manager_dict.items():
            reordered_tensor=optimizer_manager._reorder_tensors(reorder_indexes)
            dict[name]=reordered_tensor
        return dict
...
def init_ConstantLR(config, optimize_tensor, lr_list, donotuse_steps=0):
    ...
    logger.info(f"Using lr={config.init_lr}")
...
def init_ExponentialLR(config, optimize_tensor, lr_decay_steps, lr_delay_steps, lr_list, donotuse_steps=0):
    ...
    if not hasattr(config, 'gamma'):
        ...
        logger.info(f"Using init_lr={init_lr}, final_lr={final_lr}, decay_steps={lr_decay_steps}, delay_steps={lr_delay_steps}")
    else:
        init_lr=config.init_lr
        gamma=config.gamma
        logger.info(f"Using init_lr={init_lr}, gamma={gamma}")
