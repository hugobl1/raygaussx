...
timestamp = timestamp+'_mipnerf360'
...
if __name__ == "__main__":
  print("#"*80)
  scenes_indoor=["bonsai","counter","kitchen","room"]
  scenes_outdoor=["bicycle","flowers","garden","stump","treehill"]
  mean_psnr_over_iter = []
  mean_ssim_over_iter = []
  mean_lpips_over_iter = []
  mean_fps_over_iter = []
  start_time = time.time()
  for scene_to_train in scenes_indoor:
    print("#"*80)
    print(scene_to_train)
    mempool = cp.get_default_memory_pool()
    config.scene.source_path = "./dataset/mipnerf360/360_v2/" + scene_to_train
    config.scene.train_resolution_scales = [2.0]
    config.scene.test_resolution_scales = [2.0]
  ...
  print("#"*80)
  print("#"*80)
  for scene_to_train in scenes_outdoor:
    print("#"*80)
    print(scene_to_train)
    mempool = cp.get_default_memory_pool()
    config.scene.source_path = "./dataset/mipnerf360/360_v2/" + scene_to_train
    config.scene.train_resolution_scales = [4.0]
    config.scene.test_resolution_scales = [4.0]
    timestamped_dir_scene_to_train = timestamped_dir + "/" + scene_to_train
    config.save.models=os.path.join(timestamped_dir_scene_to_train, config_save_models)
    config.save.screenshots=os.path.join(timestamped_dir_scene_to_train, config_save_screenshots)
    config.save.metrics=os.path.join(timestamped_dir_scene_to_train, config_save_metrics)
  ...
  print("Mean Train time : ", ((time.time()-start_time)/60)/(len(scenes_indoor)+len(scenes_outdoor)))
