...
extern "C" __global__ void gui_sh(float* campos,float3 *positions, float* sh, 
    float* sph_gauss_features, float* bandwidth_sharpness, float* lobe_axis, 
    unsigned int num_sph_gauss,int sh_degree,
    int num_points,int max_num_sh, int max_sg_display,
    ...
    for(int l=0;l<num_sph_gauss;l++){
        float sg_x=sph_gauss_features[idx*max_sg_display*3+l];
        float sg_y=sph_gauss_features[idx*max_sg_display*3+l+max_sg_display];
        float sg_z=sph_gauss_features[idx*max_sg_display*3+l+2*max_sg_display];
        float sharpness = bandwidth_sharpness[idx*max_sg_display+l];
        float3 axis=make_float3(lobe_axis[idx*max_sg_display*3+l*3],lobe_axis[idx*max_sg_display*3+l*3+1],lobe_axis[idx*max_sg_display*3+l*3+2]);
    ...
    int idx_red=idx*max_num_sh*3;
    int idx_green=idx*max_num_sh*3+max_num_sh;
    int idx_blue=idx*max_num_sh*3+2*max_num_sh;
